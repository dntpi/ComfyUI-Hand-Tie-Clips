...
PROVENANCE. This file is not original Hand Tie Clips work and the header is
here so nobody has to reverse-engineer that from the code:

  * The cache design and implementation are **silveroxides**'
    ``ComfyUI-UtilsCollection/helpers/patcher_helpers.py`` -- ``MiniMaxH3Cache``,
    ``MiniMaxH3SamplingScope``, ``run_minimax_h3_blocks``,
  ...
  * ``h3_cache_forward`` below is a reimplementation of ComfyUI Core's
    ...
    **GPL-3.0**. See THIRD_PARTY_NOTICES.md -- that inheritance is the one part
    of this file no third party was in a position to relicense.

Redistribution here is by **permission from silveroxides, given 2026-09-16**,
with PlagueKind informed and agreeing. Attribution is the condition and it is
kept in three places: this header, ``THIRD_PARTY_NOTICES.md``, and the node's
own ``DESCRIPTION`` string. Do not strip any of them.

WHAT IT DOES. H3 is a 33B model and video sampling is many expensive steps.
When consecutive steps produce near-identical features, this reuses the last
whole-block-stack residual instead of recomputing it. That is what lets the
non-turbo base run at a usable speed, which is the entire reason Hand Tie Clips
can recommend the non-turbo path -- see docs/LEVER_AUDIT.md.

WHY IT REIMPLEMENTS A CORE FORWARD. Core exposes per-block replacements but no
replacement boundary around the *complete* block stack, and a per-block skip is
the wrong granularity. ``h3_cache_forward`` is current Core's forward with that
one missing ``block_loop`` boundary added, installed via
``ModelPatcher.add_object_patch`` on a **cloned** patcher only -- Core's model
class is never touched globally and the original bound method returns whenever
the clone is unpatched.

THE MAINTENANCE COST, STATED PLAINLY. Because this duplicates Core-internal
forward logic instead of calling into it, a Core change to
``MiniMaxH3Model._forward`` desyncs this file **silently** -- wrong output, not
an exception. ``tools/check_h3_cache.py`` exists to make that failure loud:
...
and silveroxides' original both handle it. The three restored entries are in
``has_aud_cond``, ``seg_t`` and ``seg_tag`` below.
"""
...
log = logging.getLogger("HandTieClips")

OWNER_KEY = "handtieclips_h3_minimax_cache"
# The two upstreams write their own keys into ``model_options``.
# Reading them is how a double patch is caught -- see
# ``patch_h3_minimax_cache``.
FOREIGN_OWNER_KEYS = (
    "plaguekind_h3_minimax_cache",
    "utilscollection_minimax_h3_cache",
)
...
class H3CacheState:
    ...
    def finish(self) -> None:
        if self.verbose and self.run_count + self.skip_count:
            ...
            log.info(
                "[HandTieClips] H3 Cache skipped %s/%s block-stack "
    ...
    def __call__(self, args: dict[str, Any], extra_options: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.cached_residual is not None and self.previous_feature_signature is not None:
            ...
            if below_threshold and below_skip_limit and in_cache_range:
                ...
                if self.verbose:
                    log.info(
                        "[HandTieClips] H3 Cache step %s SKIP "
                        "(relative L1 %.4f < %.4f).",
                        self.step_counter,
        ...
        if self.verbose:
            log.info(
                "[HandTieClips] H3 Cache step %s RUN (%s).",
                self.step_counter,
                skip_reason,
            )
...
def h3_cache_forward(
    ...
    **kwargs: Any,
    ...
    has_aud_cond = any(kind in ("ref_audio", "cond_audio") for _, _, kind in layout.segments)
    seg_t = {
        "text": t_v,
        "video": t_v,
        "audio": t_a,
...
def patch_h3_minimax_cache(
    ...
    # Both upstreams replace ("dit", "block_loop", 0) and object-patch the same
    # `diffusion_model._forward`, so a second cache on one wire overwrites the
    # first one's replacement while the first one's OUTER_SAMPLE wrapper
    # survives -- leaving an orphaned cache state that begins and finishes
    # around a block stack it no longer sees. Nothing raises; the run is just
    # quietly wrong. Anyone who had PlagueKind's pack installed before this node
    # existed is one stale workflow away from exactly that, so refuse rather
    # than stack.
    existing = getattr(model, "model_options", None) or {}
    already = [k for k in (OWNER_KEY,) + FOREIGN_OWNER_KEYS if existing.get(k)]
    if already:
        raise ValueError(
            "This MODEL already carries an H3 block-stack cache (%s). Two of "
            "them on one wire silently corrupt each other, so remove one. Hand "
            "Tie Clips ships its own now, so the external cache node is no "
            "longer needed in this workflow." % ", ".join(already))

    patched_model = model.clone()
    diffusion_model = patched_model.model.diffusion_model
    if not isinstance(diffusion_model, minimax_model.MiniMaxH3Model):
        raise ValueError(
            "H3 Cache requires a MiniMax H3 diffusion model; "
    ...
    if hasattr(patched_model, "model_options"):
        patched_model.model_options[OWNER_KEY] = True
