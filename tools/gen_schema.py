...
def build():
    ...
    shot = {
        ...
        "properties": {
            ...
            "anchor": {"type": "string", "enum": ["restart"],
                       "description": "Re-anchor the chain on the start image "
                                      "at this hop instead of relaying the "
                                      "previous one. Bounds the quality decay "
                                      "on a long chain: every hop otherwise "
                                      "inherits its predecessor's most settled "
                                      "frames, so motion and lighting response "
                                      "fade the further in you go. It is a CUT "
                                      "-- the hop opens on the start image's "
                                      "pose, not where the previous hop ended "
                                      "-- so pair it with join 'cut' and write "
                                      "the beat as a fresh start. Needs a start "
